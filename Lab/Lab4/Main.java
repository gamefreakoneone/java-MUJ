...
public class Main{
    public static void main(String args[]){
        ...
        int count;
        count= AddressList.countAddressWithCity(a,"Hyderabad");
        System.out.println(count);
        count= AddressList.countAddressWithPin(a,"#123");
        System.out.println(count);

        Address b1[] = AddressList.getAddressWithCity(a,"Hyderabad");
        Address b2[] = AddressList.getAddressWithPin(a,"#123");
        for(int i=0;i<b1.length;i++){
            if(b1[i]==null){
                break;
            }
            System.out.println(b1[i].getLine1()+" "+b1[i].getLine2()+" "+b1[i].getLine3()+" "+ b1[i].getCity()+" "+b1[i].getState()+" "+b1[i].getPin());
        }
        for(int i=0;i<b2.length;i++){
            if(b2[i]==null){
                break;
            }
            System.out.println(b2[i].getLine1()+" "+b2[i].getLine2()+" "+b2[i].getLine3()+" "+ b2[i].getCity()+" "+b2[i].getState()+" "+b2[i].getPin());
        }
    }
}